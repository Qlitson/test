...
所以这篇文章，其实就是带你分别把这两个框填明白。Style 框我们讲一块，歌词框我们讲三块，加起来五个维度，每讲一个，就往我们《只有你》的成品里塞一块。
...
回过头看，我们其实就干了五件事。给声音定了个调（Style 框加速查表），搭了个骨架（结构标签加哼唱前奏），当了回导演（行内标签加那段 rap），填了能唱的词（中文歌词技巧），最后把杂音踢出去（排除）。
